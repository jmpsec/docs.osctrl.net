---
title: "tui"
description: "osctrl-cli tui: full-screen terminal dashboard."
sidebar:
  order: 14
---

`osctrl-cli tui` (alias `dashboard`) opens a full-screen terminal dashboard — fleet health, platforms, environments, queries, and audit log — refreshed automatically every 30 seconds. It works in both `--db` and `--api` mode.

```properties
$ ./osctrl-cli tui -h
NAME:
   osctrl-cli tui - Full-screen TUI dashboard — fleet health, platforms, environments, queries, audit

USAGE:
   osctrl-cli tui [options]
...
```shell
$ ./osctrl-cli tui --db -D db.yml
$ ./osctrl-cli dashboard --api -A osctrl-api.json
```
